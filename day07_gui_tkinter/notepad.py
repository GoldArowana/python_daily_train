...
from tkinter import filedialog,simpledialog,colorchooser,font
import fileinput
from tkinter import *
import os
...
def about():
    messagebox.showinfo(title="当前版本为1.0,欢迎使用", message="**notepad")
...
class editor():
    ...
    def savefile(self, event=None):
        if os.path.isfile(self.t.title()):
            ...
            sname = filedialog.asksaveasfilename(title="保存好你的文档哟", filetypes=[("保存文件", "*.txt")],
    ...
    def saveasfile(self, event=None):
        sname = filedialog.asksaveasfilename(title="保存好你的文档哟", filetypes=[("保存文件", "*.txt")], defaultextension=".txt")
        if sname:
            ofp = open(sname, "w")
            ofp.write(self.st.get(1.0, tk.END))
            ofp.flush()
